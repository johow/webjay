...
files2folder(input, src_path=input$src_folder, dest_path=input$my_folder)
 ...
 if(input$dropbox==TRUE){drop_folder(from=file.path(input$my_folder, "out"), to=input$dropbox_folder)}
}
)

shinyUI(fluidPage(
  titlePanel("Webjay Version 0.3"),
  sidebarLayout(
    sidebarPanel(
    ...
    h2("Specify Header"),
      ...
      textInput("submenu", "Submenu elements", value = "vita newsfeeds impressum about")), inputPanel( 
...
textInput("b_wid", "Buttons Widths", value = "10 10 20 20 10"),
 ...
 checkboxInput("ftp", "Upload via FTP", value = FALSE),
  ...
  passwordInput("ftp_pw", "FTP Password", value="oav_23gS")), 
         inputPanel(
  h2("Dropbox"), 
 checkboxInput("dropbox", "Upload to Dropbox", value = TRUE),
